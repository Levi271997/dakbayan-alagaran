...
$start_from = ($page-1) * $limit;  
        ...
        if($level == "super admin") {
            $query="SELECT tblregistration.*, (SELECT zone_name from tbl_zones where tbl_zones.zone_id=tblregistration.zone) as street_name, barangay.* FROM tblregistration INNER JOIN barangay ON tblregistration.baranggay = barangay.id WHERE member_status ='2' LIMIT $start_from, $limit";
        } else if($level == "coordinator") {
            $query="SELECT tblregistration.*,(SELECT zone_name from tbl_zones where tbl_zones.zone_id=tblregistration.zone) as street_name,  barangay.* FROM tblregistration INNER JOIN barangay ON tblregistration.baranggay = barangay.id WHERE member_status ='2' AND personnel_id = '$coorid' LIMIT $start_from, $limit";
        }
    ...
    if(mysqli_num_rows($rec) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($rec)) {
            ...
            echo "<td>";
            echo "<span class='tablebuttons primary mp makepayments' data-bs-toggle='modal' data-bs-target='#staticBackdrop3' data-id='".$id."'></span>";
           ...
           if($level ==="super admin"){
            ...
            // echo "<td><div id='activate' class='btn btn-primary' data-id=".$id."> Activate </div> </td>";
  ...
  $limit = 14;
  if($level == "super admin"){
    $sql = "SELECT COUNT(uid) FROM tblregistration WHERE member_status='2'";  
  }else{
    $sql = "SELECT COUNT(uid) FROM tblregistration WHERE member_status='2' AND personnel_id = '".$coorid."'";  
  }
