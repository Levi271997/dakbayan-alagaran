...
$start_from = ($page-1) * $limit;  
        ...
        if($level == "super admin"){
            ...
            ,barangay.* FROM tblregistration INNER JOIN barangay ON tblregistration.baranggay = barangay.id WHERE member_status ='3' LIMIT $start_from, $limit";
        }else{
            $query="SELECT tblregistration.*,(SELECT zone_name from tbl_zones where tbl_zones.zone_id=tblregistration.zone) as street_name
            ,barangay.* FROM tblregistration INNER JOIN barangay ON tblregistration.baranggay = barangay.id WHERE member_status ='3' AND personnel_id = '$coorid' LIMIT $start_from, $limit";
        }
    ...
    if(mysqli_num_rows($rec) > 0){
        ...
        while ($row = mysqli_fetch_assoc($rec)) {
            ...
            if($level == "super admin"){
                echo "<td><div id='activate' class='btnactivate' data-id=".$id."> Activate </div> </td>";
  ...
  $limit = 14;
  $sql = "SELECT COUNT(uid) FROM tblregistration WHERE member_status='3'";  
